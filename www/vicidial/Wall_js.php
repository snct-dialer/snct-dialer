...
# 
# Copyright (C) 2016 Matt Florell <vicidial@gmail.com>    LICENSE: AGPLv3
#               2017 Jörg Frings-Fürst <jff@flyingpenguin.de>
...
#		header('HTTP/1.1 503 Too busy, try again later');
	...
	echo "<br>Bitte verwenden Sie für den Aufruf dieses Wallboards nur den Server ViciDB2 (172.16.2.13)" . PHP_EOL;
}
...
if ($RTajax < 1)
	...
	echo "<a href=\"./AST_timeonVDADallSUMMARY_ng.php?RR=$RR&DB=$DB&adastats=$adastats\">"._QXZ("SUMMARY")."</a> </FONT>\n";
	echo "\n\n";
	}
...
#	if($DAleads > 20) {
#		$FDL = "bgcolor='#ff6666' <font size=4";
#	}
#	else {
#		$FDL = "<font size=2";
#	}
#	echo "<TD $FDL ALIGN=RIGHT><B>"._QXZ("DIALABLE LEADS").":</B></TD><TD $FDL ALIGN=LEFT>&nbsp; $DAleads &nbsp; &nbsp; </TD>";
#	echo "<TD ALIGN=RIGHT><font size=2><B> &nbsp; </TD><TD ALIGN=LEFT><font size=2> &nbsp </TD>";
#	echo "<TD ALIGN=RIGHT><font size=2><B> "._QXZ("TIME").":</B> &nbsp; </TD><TD ALIGN=LEFT><font size=2> $NOW_TIME </TD>";
#	echo "</TR>";
...
$Cecho = '';
$Cecho .= "<table cellpadding=1 cellspacing=1 border=0>";
$Cecho .= "<tr>";
...
#$Cecho .= "+---------+----------------------+--------------------+-----------------+---------+------------+----------+\n";
#$Cecho .= "| "._QXZ("STATUS",7)." | "._QXZ("CAMPAIGN",20)." | "._QXZ("PHONE NUMBER",18)." | "._QXZ("SERVER IP",15)." | "._QXZ("DIALTIME",8)."| "._QXZ("CALL TYPE",10)." | "._QXZ("PRIORITY",8)." | $agentonlyheader\n";
#$Cecho .= "+---------+----------------------+--------------------+-----------------+---------+------------+----------+\n";
...
while($p<$k)
	...
	$Cphone_number =	sprintf("%-18s", $CDphone_number[$p]); # Do not translate
...
###################################################################################
	...
	if ($report_display_type=='HTML')
	{
		...
		$HTuser =  "<td NOWRAP><font class='top_head_key'>&nbsp; "._QXZ("USER")." </td>";
		$HDusergroup =		"";
		...
		if ($RTajax > 0)
		{$HTtime =	"<td NOWRAP>&nbsp;<font class='top_head_key'>MM:SS&nbsp;</td>";}
		$HDcampaign =		"";
		...
		if ($RTajax > 0)
		{$HTcampaign =	"<td NOWRAP><font class='top_head_key'>"._QXZ("CAMPAIGN",10)."&nbsp;</td>";}
		$HDcalls =			"";
	...
	{
		$Aline  = "$HDbegin$HDusergroup$HDcalls$HDsessionid$HDbarge$HDwhisper$HDstatus$HDpause$HDcustphone$HDserver_ip$HDcall_server_ip$HDtime$HDcampaign$HDigcall\n";
		$Bline  = "$HTbegin$HTusergroup$HTcalls$HTsessionid$HTbarge$HTwhisper$HTstatus$HTpause$HTcustphone$HTserver_ip$HTcall_server_ip$HTtime$HTcampaign$HTigcall\n";
	}
	else
	{
#		$Aline  = "$HDbegin$HDphone$HDuser$HDcalls$HDusergroup$HDbarge$HDwhisper$HDstatus$HDtime$HDpause$HDcustphone$HDserver_ip$HDcall_server_ip$HDcampaign$HDstation$HDsessionid$HDigcall\n";
#		$Bline  = "$HTbegin$HTphone$HTuser$HTcalls$HTusergroup$HTbarge$HTwhisper$HTstatus$HTtime$HTpause$HTcustphone$HTserver_ip$HTcall_server_ip$HTcampaign$HTstation$HTsessionid$HTigcall\n";
		
		$Aline  = "$HDbegin$HDphone$HDuser$HDcalls$HDusergroup$HDbarge$HDwhisper$HDstatus$HDtime$HDpause$HDigcall$HDcustphone$HDserver_ip$HDcall_server_ip$HDcampaign$HDstation$HDsessionid\n";
		$Bline  = "$HTbegin$HTphone$HTuser$HTcalls$HTusergroup$HTbarge$HTwhisper$HTstatus$HTtime$HTpause$HTigcall$HTcustphone$HTserver_ip$HTcall_server_ip$HTcampaign$HTstation$HTsessionid\n";
	}
	$Aecho .= "$Aline";
	$Aecho .= "$Bline";
	$Aecho .= "$Aline";
		...
		while ($j < $talking_to_print)
		{
			...
			if ($Lstatus=='PAUSED')
			{
				...
				else
				{
					...
					if ($call_time_S >= 60) {$G='<SPAN class="yellow"><B>'; $EG='</B></SPAN>'; $tr_class='TRyellow';}
					if ($call_time_S >= 300) {$G='<SPAN class="olive"><B>'; $EG='</B></SPAN>'; $tr_class='TRolive';}
					if ($call_time_S >= 3600) {$G='<SPAN class="red"><B>'; $EG='</B></SPAN>'; $tr_class='TRred';}
				}
			}
...
#				if ($CUSTPHONEdisplay > 0)	{$CP = " $G$custphone$EG </td><td NOWRAP align=right>";}
				if ($CUSTPHONEdisplay > 0)	{$CP = "<td NOWARP align=right> $G$custphone$EG </td>";}
				else	{$CP = "";}
...
#				if ($agent_pause_codes_active > 0)	{$pausecodeHTML = "<td bgcolor=white align=center><font class='Hblank'>$pausecode</td>";}
				if ($agent_pause_codes_active > 0)	{$pausecodeHTML = "<td align=center>$G$pausecode$EG</td>";}
				else	{$pausecodeHTML = "";}
				...
				if ($realtime_block_user_info < 1)
				{
					$Aecho .= "<tr class='$tr_class'><td NOWRAP>$phoneD$G$user$EG</td><td NOWRAP align=right> $G$calls_today$EG </td><td NOWRAP align=center> $G"._QXZ("$status",6)."$EG</td><td align=center>$G$CM$EG</td><td NOWRAP align=right>$SVD$G$call_time_MS$EG </td>$pausecodeHTML $INGRP$CP<td NOWRAP align=right> $G$campaign_id$EG </td><td NOWRAP> $G$extension$Aring_note[$i]$EG</td><td NOWRAP align=right>$UGD $G$sessionid$EG$L$R </td></tr>\n";
#					$Aecho .= "<tr class='$tr_class'><td NOWRAP>$phoneD$G$user$EG</td><td NOWRAP align=right> $G$calls_today$EG </td><td NOWRAP align=center> $G"._QXZ("$status",6)."$EG</td><td align=center>$G$CM$EG</td><td NOWRAP align=right>$SVD$G$call_time_MS$EG </td>$pausecodeHTML$CP<td NOWRAP align=right> $G$campaign_id$EG </td><td NOWRAP> $G$extension$Aring_note[$i]$EG</td><td NOWRAP align=right>$UGD $G$sessionid$EG$L$R </td>$INGRP</tr>\n";
				}
			}
			$j++;
		}
	...
	#	$Aecho .= "  <SPAN class=\"red\"><B>          </SPAN> - "._QXZ("Agent chatting")."</B>\n";
		...
		$Aecho .= "  <SPAN class=\"red\"><B>          </SPAN> - "._QXZ("Agent Paused > 60 minutes")."</B>\n";
		...
		if ($ring_agents > 0)
			...
			$Aecho .= "  <SPAN><B>"._QXZ("Version").": " .$release. "</B></SPAN>\n";
			}
...
#		echo "$NFB$B &nbsp;$agent_ready $BG$NFE "._QXZ("agent(s)) waiting")." &nbsp; &nbsp; &nbsp; \n";
...
#		echo "$NFB$F &nbsp;$out_live $FG$NFE "._QXZ("calls waiting for agents")." &nbsp; &nbsp; &nbsp; \n";
	    ...
	    $agent_inwork = $agent_dispo + $agent_dead;
		$FPA = '<FONT class="gr1">'; $BG='</FONT>';
		if ($agent_total > 0) {
		    $test = $agent_paused / $agent_total;
#		    echo "$test\n";
		    if($test >= 0.6) {
			$FPA = '<FONT class="rd1">'; $BG='</FONT>';
		    }
		    else {
			if($test >= 0.4) {
			    $FPA = '<FONT class="ge1">'; $BG='</FONT>';
			}
		    }
		}
		$FAD = '<FONT class="gr1">'; $BG='</FONT>';
		$agent_act = $agent_total - $agent_paused;
		if($agent_act > 0) {
		    $test = $agent_inwork / $agent_act;
#		    echo "$test\n";
		    if($test >= 0.6) {
			$FAD = '<FONT class="rd1">'; $BG='</FONT>';
		    }
		    else {
			if($test >= 0.4) {
			    $FAD = '<FONT class="ge1">'; $BG='</FONT>';
			}
		    }
		...
		$D1echo =  "<TABLE width='100%'>\n";
		$D1echo .= " <TR> <TD>\n";
		$D1echo .= " <TABLE cellpadding='1' cellspacing='1' frame='box'>\n";
		$D1echo .= "  <TR>\n";
		$D1echo .= "   <TD> $NFB$F &nbsp;$out_live $FG$NFE "._QXZ("calls waiting for agents")." &nbsp; &nbsp; &nbsp; </TD>\n";
		$D1echo .= "   <TD> $NFB$B &nbsp;$agent_ready $BG$NFE "._QXZ("agents waiting")." &nbsp; &nbsp; &nbsp; </TD>\n";
		$D1echo .= "  </TR>\n";
		$D1echo .= "  <TR>\n";
		$D1echo .= "   <TD> $NFB$FPA &nbsp;$agent_paused $BG$NFE "._QXZ("paused agents")." &nbsp; &nbsp; &nbsp; </TD>\n";
		$D1echo .= "   <TD> $NFB$FAD &nbsp;$agent_inwork $BG$NFE "._QXZ("agents in dispo / dead")."&nbsp; &nbsp; &nbsp; </TD>\n";
		$D1echo .= "  </TR\n";
#		$D1echo .= " </TABLE>\n";
#		$D1echo .= " </TD></TR>\n";
#		$D1echo .= "<p><br><br></p>\n";
#		$D1echo .= " <TR><TD>\n";
#		$D1echo .= " </TD>\n";
#		$D1echo .= " <TD style='width:50%;'>\n";
#		$D1echo .= " <TABLE  cellpadding='1' cellspacing='1' frame='box'>\n";
		$D1echo .= "  <TR>\n";
		$D1echo .= "   <TD> $NFB$out_ring$NFE "._QXZ("calls ringing")." &nbsp; &nbsp; &nbsp; &nbsp; </TD>\n";
		$D1echo .= "   <TD> $NFB$out_total$NFE "._QXZ("current active calls")."&nbsp; &nbsp; &nbsp; </TD>\n";
		$D1echo .= "  </TR>\n";
		$D1echo .= "  <TR>\n";
		$D1echo .= "   <TD> $NFB$agent_total$NFE "._QXZ("agents logged in")." &nbsp; &nbsp; &nbsp; &nbsp; </TD>\n";
		$D1echo .= "   <TD> $NFB$agent_incall$NFE "._QXZ("agents in calls")." &nbsp; &nbsp; &nbsp; </TD>\n";
		$D1echo .= "  </TR>\n";
		$D1echo .= " </TABLE>\n";
		$D1echo .= " <div class='clear'></div>\n";
		$D1echo .= "<BR><p>\n";
		$D1echo .= " </TD> </TR>\n";
		$D1echo .= "</TABLE>\n";
		
		$D1echo .= "<TABLE cellpadding=0 cellspacing=0><TR>";
		if($DAleads > 20) {
			$FDL = "bgcolor='#ff6666' <font size=4";
		}
		else {
		...
		$D1echo .= "<TD $FDL ALIGN=RIGHT><B>"._QXZ("DIALABLE LEADS").":</B></TD><TD $FDL ALIGN=LEFT>&nbsp; $DAleads &nbsp; &nbsp; </TD>";
		$D1echo .= "<TD ALIGN=RIGHT><font size=2><B> &nbsp; </TD><TD ALIGN=LEFT><font size=2> &nbsp </TD>";
		$D1echo .= "<TD ALIGN=RIGHT><font size=2><B> "._QXZ("TIME").":</B> &nbsp; </TD><TD ALIGN=LEFT><font size=2> $NOW_TIME </TD>";
		$D1echo .= "</TR>";
		$D1echo .= "</TABLE>";
		
		
		$D1echo .= "<PRE><FONT SIZE=2>";
#		$D1echo .= "";
		...
		if($row[$LeadingName . "Calls"] != 0.0) {
			...
			$D3echo .= "<TABLE cellpadding='1' cellspacing='1' rules='all' frame='box'>\n";
...
//			$D3echo .= "  <TD align='center'> ". number_format (($row[7] - $row[8] - $row[9]) , 0 , ',' , '.' )."</TD>\n";
		...
		echo "  <TR valign='top'><TD style='vertical-align:text-top'>\n";
		echo "$D1echo";
		echo "$D3echo";
