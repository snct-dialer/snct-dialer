...
##	$efp = fopen ("./functions_debug.log", "a");
	if($return == 1) {
		$return = 0;
...
#		fwrite ($efp, "$NOW_TIME|".$stmt."|\n");
		...
		if ($DB) {echo "$stmt\n";}
		$AM_ct = mysqli_num_rows($rslt1);
		if ($AM_ct > 0)
		{
		    $row1=mysqli_fetch_array($rslt1, MYSQLI_BOTH);
		    $Group_Mail = $row1[0];
		    $OwnTicketMail = $row1[1];
		    if($Group_Mail != "") {
		        $return = 1;
		    }
		    
##		    $efp = fopen ("./functions_debug.log", "a");
##		    fwrite ($efp, "$NOW_TIME|".$Agent."|".$row1["group_email"]."|".$row1["own_ticketmail"]."|\n");
##		    fclose($efp);
		}
	}
