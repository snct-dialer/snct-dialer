#!/usr/bin/perl
#
# AST_VDsales_export_SFTPSSH2.pl                version: 2.4
#
...
# 120228-1855 - Added SFTP(SSH2) file transmission option
#
...
if (length($ARGV[0])>1)
	...
	if ($args =~ /--help/i)
		...
		print "  [--sftp-mode] = Uses SFTP(SSH2) for 'ftp' options\n";
...
if ($ftp_transfer > 0)
	...
	if ($sftp_mode > 0)
		...
		if ($DB > 0)
			{$ssh2->debug("1");}
		$ssh2->connect("$VARREPORT_host","$VARREPORT_port") or die "Unable to connect Host $@ \n";
...
if ($ftp_audio_transfer > 0)
	...
	foreach(@FILES)
		...
		if ( (length($FILES[$i]) > 4) && (!-d "$tempdir/$FILES[$i]") )
			...
			if ($sftp_mode > 0)
				{
				if (!$Q) {print "Sending Audio Over SFTP: $FILES[$i]\n";}
				use Net::SSH2::SFTP;
				use Data::Dumper;
				my $ssh2 = Net::SSH2->new();
				if ($DB > 0)
				...
				$ssh2->auth_password("$VARREPORT_user","$VARREPORT_pass") or die "Unable to login $@ \n";
				my $sftp = $ssh2->sftp();
				$newVARREPORT_dir = $VARREPORT_dir;
				if ($NODATEDIR < 1)
					{
					if ($YEARDIR > 0)
						{
						if ($DB > 0) {print "making directory: $VARREPORT_dir/$year\n";}
						$sftp->mkdir("$VARREPORT_dir/$year");
						$newVARREPORT_dir = "$VARREPORT_dir/$year/$start_date";
						}
					else
						{
						$newVARREPORT_dir = "$VARREPORT_dir/$start_date";
						}
					if ($DB > 0) {print "making directory: $newVARREPORT_dir\n";}
					$sftp->mkdir("$newVARREPORT_dir");
					my %stat = $sftp->stat($newVARREPORT_dir);
					print Dumper([\%stat]), "\n";
					}
				$ssh2->scp_put("$tempdir/$FILES[$i]", "$newVARREPORT_dir/$FILES[$i]");
				$ssh2->disconnect();
				}
			else
				{
